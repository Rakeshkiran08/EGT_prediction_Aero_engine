...
load aero.txt;                                           %% loading of the dataset
L = input('Enter the number of inputs (MAX 9) ');
...
random1 = randi([1,size(aero,1)],1,size(aero,1));        %% radomizing the input dataset
for i=1:length(random1)
    data1(i,:) = aero(random1(i),:);
end
